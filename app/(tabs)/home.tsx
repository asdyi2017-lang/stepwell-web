import { Activity, AlertTriangle, Droplets, HeartPulse, Stethoscope, LogOut, Watch, WifiOff, Pencil, MessageCircle, Send, Download, Sun, Footprints, Flame, Zap, Award, BarChart2, X, Share2, Heart, Target, Settings, ChevronRight } from 'lucide-react-native';
            ...
            <ScrollView contentContainerStyle={{ paddingBottom: 24, gap: 20 }} showsVerticalScrollIndicator={false}>
              ...
              <View>
                <ProfileMenuItem 
                  ...
                  onPress={() => Alert.alert('Health Info', `Height: ${user?.height || '-'} cm\nWeight: ${user?.weight || '-'} kg\nDate of Birth: ${user?.date_of_birth ? new Date(user.date_of_birth).toLocaleDateString() : '-'}`)} 
                />
                <ProfileMenuItem 
                  icon={<Target size={20} color={colors.text} />} 
                  title="My goals" 
                  onPress={() => Alert.alert('My Goals', 'Your primary goal is 10,000 steps per day. Keep moving!')} 
                />
                <ProfileMenuItem 
                  icon={<Settings size={20} color={colors.text} />} 
                  title="Settings" 
                  onPress={() => Alert.alert('Settings', 'Account settings will be available in future updates.')} 
                />
              </View>
            </ScrollView>
          ) : page === 'assistant' ? (
...
const styles = StyleSheet.create({
  ...
  profileCard: { backgroundColor: '#81B2E0', borderRadius: 24, padding: 20, flexDirection: 'row', alignItems: 'center', gap: 16 },
  ...
  profileEmail: { fontSize: 13, fontWeight: '700', color: 'rgba(255,255,255,0.8)' },
  profileMenuItem: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#D4E6F7', padding: 18, borderRadius: 16, marginBottom: 12 },
