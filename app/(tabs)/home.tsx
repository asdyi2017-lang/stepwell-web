import { Activity, AlertTriangle, Droplets, HeartPulse, Stethoscope, LogOut, Watch, WifiOff, Pencil, MessageCircle, Send, Download, Sun, Footprints, Flame, Zap, Award, BarChart2, X, Share2, Heart, Target, Settings, ChevronRight, Check, Image as ImageIcon } from 'lucide-react-native';
...
const DEFAULT_AVATAR = "https://avatars.mds.yandex.net/get-yapic/69015/3vULTgBvet3oDps8eGudxrUec-1/orig";
...
export default function Home() {
  ...
  const [avatarModal, setAvatarModal] = useState({ visible: false, value: '' });

  const [prefs, setPrefs] = useState({ length: 'metric', weight: 'metric', temp: 'celsius' });
  ...
  useEffect(() => {
    ...
    AsyncStorage.getItem('userAvatar').then(savedAvatar => {
  ...
  async function handleSaveEdit() {
    ...
    if (editModal.title === 'Blood Pressure') {
      ...
      const inputW = Number(val);
      payload.weight = prefs.weight === 'imperial' ? Number((inputW / 2.20462).toFixed(1)) : inputW;
  ...
  const handleSaveAvatar = () => {
    const newAvatar = avatarModal.value.trim();
    if (newAvatar) {
      setAvatar(newAvatar);
      AsyncStorage.setItem('userAvatar', newAvatar);
    } else {
      setAvatar(DEFAULT_AVATAR);
      AsyncStorage.removeItem('userAvatar');
    }
    setAvatarModal({ visible: false, value: '' });
  };
...
Generated by StepWell System
  ...
  const displayTemp = weather ? (prefs.temp === 'fahrenheit' ? (weather.temp * 9/5 + 32).toFixed(1) : weather.temp) : null;
  const tempUnit = prefs.temp === 'fahrenheit' ? '°F' : '°C';
  
  const wVal = snapshot?.weight?.value;
  const displayW = wVal ? (prefs.weight === 'imperial' ? (wVal * 2.20462).toFixed(1) : wVal) : "-";
  const wUnit = prefs.weight === 'imperial' ? 'lbs' : 'kg';
  ...
  function ProfileMenuItem({ icon, title, onPress }: any) {
    return (
      <Pressable style={styles.profileMenuItem} onPress={onPress}>
      ...
      <View style={styles.root}>
        ...
        <View style={styles.main}>
          <View style={styles.header}>
            <View style={{ flex: 1 }}>
              ...
              <View style={{flexDirection: 'row', alignItems: 'center', gap: 8, flexWrap: 'wrap'}}>
                <Text style={styles.hSub}>
                  {page === 'profile' ? 'Manage your account settings' : currentRole === 'doctor' ? 'Healthcare provider view' : 'Today summary • alerts'}
                </Text>
                {currentRole === 'doctor' && selectedPatientId && page !== 'profile' && (
                ...
                {currentRole !== 'doctor' && snapshot && page !== 'profile' && (
                  <Pressable onPress={downloadReport} style={styles.downloadIconBtn}>
                    <Download size={14} color={colors.teal} />
            ...
            <Pressable onPress={() => setPage('profile')} style={[styles.avatarWrap, {marginLeft: 10}]}>
            ...
            <ScrollView contentContainerStyle={{ paddingBottom: 24, gap: 20 }} showsVerticalScrollIndicator={false}>
              <View style={styles.profileCard}>
                <View style={{position: 'relative'}}>
                  <Image source={{ uri: avatar }} style={styles.profileBigAvatar} />
                  <Pressable 
                    style={styles.editAvatarBtn} 
                    onPress={() => setAvatarModal({visible: true, value: avatar !== DEFAULT_AVATAR ? avatar : ''})}
                  >
              ...
              <View>
                <ProfileMenuItem 
                  ...
                  onPress={() => {
                    const h = user?.height;
                    const w = user?.weight;
                    const displayHInfo = h ? (prefs.length === 'imperial' ? (h / 30.48).toFixed(1) + ' ft' : h + ' cm') : '-';
                    const displayWInfo = w ? (prefs.weight === 'imperial' ? (w * 2.20462).toFixed(1) + ' lbs' : w + ' kg') : '-';
                    
                    setInfoModal({ 
                      visible: true, 
                      title: 'Health Information', 
                      content: `Height: ${displayHInfo}\nWeight: ${displayWInfo}\nDate of Birth: ${user?.date_of_birth ? new Date(user.date_of_birth).toLocaleDateString() : '-'}` 
                    });
                  }} 
                />
                <ProfileMenuItem 
                  icon={<Target size={20} color={colors.teal} />} 
                  title="My goals" 
                  onPress={() => setInfoModal({ 
                    visible: true, 
                    title: 'My Goals', 
                    content: 'Your primary goal is 10,000 steps per day. Keep moving and stay hydrated!' 
                  })} 
                />
                <ProfileMenuItem 
                  icon={<Settings size={20} color={colors.teal} />} 
                  title="Settings" 
            ...
            <ScrollView contentContainerStyle={{ paddingBottom: 24 }} showsVerticalScrollIndicator={false}>
              ...
              {page === 'dashboard' && snapshot && currentRole !== 'doctor' && weather && (
                <View style={styles.weatherCard}>
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={styles.weatherTitle}>Aktobe • {displayTemp}{tempUnit}</Text>
              ...
              {page === 'dashboard' && snapshot && (
                <View style={[styles.grid, !isDesktop && styles.gridMobile]}>
                  ...
                  <BiomarkerCard
                    ...
                    showEdit={currentRole === 'doctor'}
                    onEdit={() => setEditModal({ visible: true, title: 'Weight', value: String(snapshot.weight.value ? (prefs.weight === 'imperial' ? (snapshot.weight.value * 2.20462).toFixed(1) : snapshot.weight.value) : "") })}
                  />
                </View>
              )}
              ...
              {page === 'statistics' && snapshot && currentRole !== 'doctor' && (
                <View style={{ gap: 16 }}>
                  ...
                  <View style={[styles.grid, !isDesktop && styles.gridMobile]}>
                    <View style={[styles.cardBase, isDesktop ? { flex: 1, minWidth: '45%' } : { width: '100%' }]}>
                      ...
                      <View style={[styles.grid, !isDesktop && styles.gridMobile]}>
                        {patients.map((patient) => {
                          ...
                          return (
                            <Pressable 
                              key={patient.id} 
                              style={[styles.cardBase, isDesktop ? { flex: 1, minWidth: '45%' } : { width: '100%' }, levelCardStyle(pStatus)]} 
                              onPress={() => {
        ...
        <Modal visible={infoModal.visible} transparent animationType="fade">
          <View style={styles.modalOverlay}>
            <View style={styles.modalContent}>
              ...
              <Text style={[styles.modalSub, {fontSize: 15, lineHeight: 22, color: colors.text, marginBottom: 24}]}>{infoModal.content}</Text>
              
              <Pressable style={styles.btnSave} onPress={() => setInfoModal({ visible: false, title: '', content: '' })}>
          ...
          <View style={styles.modalOverlay}>
            <View style={styles.modalContent}>
              ...
              <View style={styles.settingsSection}>
                <Text style={styles.settingsLabel}>Length (Height)</Text>
                <View style={styles.toggleRow}>
                  <Pressable style={[styles.toggleBtn, prefs.length === 'metric' && styles.toggleBtnActive]} onPress={() => setPrefs({...prefs, length: 'metric'})}>
                    {prefs.length === 'metric' && <Check size={14} color={colors.teal} style={{marginRight: 6}} />}
                    <Text style={[styles.toggleText, prefs.length === 'metric' && styles.toggleTextActive]}>cm</Text>
                  </Pressable>
                  <Pressable style={[styles.toggleBtn, prefs.length === 'imperial' && styles.toggleBtnActive]} onPress={() => setPrefs({...prefs, length: 'imperial'})}>
                    {prefs.length === 'imperial' && <Check size={14} color={colors.teal} style={{marginRight: 6}} />}
                    <Text style={[styles.toggleText, prefs.length === 'imperial' && styles.toggleTextActive]}>ft</Text>
                  </Pressable>
                </View>
              </View>

              <View style={styles.settingsSection}>
                <Text style={styles.settingsLabel}>Weight</Text>
                <View style={styles.toggleRow}>
                  <Pressable style={[styles.toggleBtn, prefs.weight === 'metric' && styles.toggleBtnActive]} onPress={() => setPrefs({...prefs, weight: 'metric'})}>
                    {prefs.weight === 'metric' && <Check size={14} color={colors.teal} style={{marginRight: 6}} />}
                    <Text style={[styles.toggleText, prefs.weight === 'metric' && styles.toggleTextActive]}>kg</Text>
                  </Pressable>
                  <Pressable style={[styles.toggleBtn, prefs.weight === 'imperial' && styles.toggleBtnActive]} onPress={() => setPrefs({...prefs, weight: 'imperial'})}>
                    {prefs.weight === 'imperial' && <Check size={14} color={colors.teal} style={{marginRight: 6}} />}
                    <Text style={[styles.toggleText, prefs.weight === 'imperial' && styles.toggleTextActive]}>lbs</Text>
                  </Pressable>
                </View>
              </View>

              <View style={styles.settingsSection}>
                <Text style={styles.settingsLabel}>Temperature</Text>
                <View style={styles.toggleRow}>
                  <Pressable style={[styles.toggleBtn, prefs.temp === 'celsius' && styles.toggleBtnActive]} onPress={() => setPrefs({...prefs, temp: 'celsius'})}>
                    {prefs.temp === 'celsius' && <Check size={14} color={colors.teal} style={{marginRight: 6}} />}
                    <Text style={[styles.toggleText, prefs.temp === 'celsius' && styles.toggleTextActive]}>Celsius</Text>
                  </Pressable>
                  <Pressable style={[styles.toggleBtn, prefs.temp === 'fahrenheit' && styles.toggleBtnActive]} onPress={() => setPrefs({...prefs, temp: 'fahrenheit'})}>
                    {prefs.temp === 'fahrenheit' && <Check size={14} color={colors.teal} style={{marginRight: 6}} />}
                    <Text style={[styles.toggleText, prefs.temp === 'fahrenheit' && styles.toggleTextActive]}>Fahrenheit</Text>
                  </Pressable>
                </View>
              </View>

              <Pressable style={[styles.btnSave, {marginTop: 10}]} onPress={() => setSettingsModal(false)}>
        ...
        <Modal visible={avatarModal.visible} transparent animationType="fade">
          <View style={styles.modalOverlay}>
            <View style={styles.modalContent}>
              ...
              <Text style={styles.modalSub}>
                Paste a direct image URL (jpg, png) below to update your profile picture.
              </Text>
              
              <TextInput
                style={[styles.input, {fontSize: 14}]}
                value={avatarModal.value}
                onChangeText={(text) => setAvatarModal({ ...avatarModal, value: text })}
                placeholder="https://example.com/image.jpg"
                autoCapitalize="none"
              />

              <View style={styles.modalActions}>
                <Pressable style={styles.btnCancel} onPress={() => setAvatarModal({ visible: false, value: '' })}>
                  <Text style={styles.btnCancelText}>Cancel</Text>
                </Pressable>
                <Pressable style={styles.btnSave} onPress={handleSaveAvatar}>
                  <Text style={styles.btnSaveText}>Update</Text>
                </Pressable>
              </View>
              
              <Pressable style={{marginTop: 16, alignItems: 'center'}} onPress={() => {
                setAvatar(DEFAULT_AVATAR);
                AsyncStorage.removeItem('userAvatar');
                setAvatarModal({ visible: false, value: '' });
              }}>
                <Text style={{fontSize: 12, fontWeight: '700', color: colors.muted}}>Reset to Default Avatar</Text>
              </Pressable>
            </View>
          </View>
        </Modal>
        ...
        <Modal visible={editModal.visible} transparent animationType="fade">
          <View style={styles.modalOverlay}>
            <View style={styles.modalContent}>
              ...
              <Text style={styles.modalSub}>
                {editModal.title === 'Blood Pressure' ? 'Enter as SYS/DIA (e.g. 120/80)' : 
                 editModal.title === 'Weight' ? `Enter numerical value (${prefs.weight === 'imperial' ? 'lbs' : 'kg'}):` :
...
function BiomarkerCard(props: { isDesktop?: boolean; title: string; value: string; meta: string; status: Level; deviceStatus: DeviceStatus; icon: React.ReactNode; progress: number; showEdit?: boolean; onEdit?: () => void; }) {
  const { isDesktop, title, value, meta, status, deviceStatus, icon, progress, showEdit, onEdit } = props;
  return (
    <View style={[styles.cardBase, isDesktop ? { flex: 1, minWidth: '45%' } : { width: '100%' }]}>
      <View style={styles.cardTop}>
        <View style={{ flexDirection: 'row', gap: 10, alignItems: 'center', flex: 1 }}>
...
const styles = StyleSheet.create({
  ...
  cardBase: { backgroundColor: '#fff', borderRadius: 20, padding: 16, borderWidth: 1.5, borderColor: colors.border },
  ...
  profileCard: { backgroundColor: colors.teal, borderRadius: 24, padding: 20, flexDirection: 'row', alignItems: 'center', gap: 16 },
  profileBigAvatar: { width: 70, height: 70, borderRadius: 35, borderWidth: 3, borderColor: '#fff' },
  profileName: { fontSize: 22, fontWeight: '900', color: '#fff', marginBottom: 4 },
  profileEmail: { fontSize: 13, fontWeight: '700', color: colors.tealSoft },
  profileMenuItem: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#fff', borderWidth: 1.5, borderColor: '#DCEAEC', padding: 18, borderRadius: 16, marginBottom: 12 },
  profileMenuLeft: { flexDirection: 'row', alignItems: 'center', gap: 12 },
  profileMenuText: { fontSize: 15, fontWeight: '800', color: colors.text },
  
  settingsSection: { marginBottom: 20 },
  settingsLabel: { fontSize: 13, fontWeight: '800', color: colors.teal, marginBottom: 8 },
  toggleRow: { flexDirection: 'row', backgroundColor: '#F8FAFC', borderRadius: 16, borderWidth: 1.5, borderColor: '#DCEAEC', padding: 4 },
  toggleBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 10, borderRadius: 12 },
  toggleBtnActive: { backgroundColor: colors.tealSoft, borderWidth: 1.5, borderColor: '#CBE3E6', shadowColor: '#000', shadowOffset: {width: 0, height: 1}, shadowOpacity: 0.05, shadowRadius: 2, elevation: 1 },
  toggleText: { fontSize: 13, fontWeight: '700', color: colors.muted },
  toggleTextActive: { color: colors.teal, fontWeight: '900' },
  
  editAvatarBtn: { position: 'absolute', bottom: -5, right: -5, backgroundColor: colors.teal, width: 26, height: 26, borderRadius: 13, alignItems: 'center', justifyContent: 'center', borderWidth: 2, borderColor: '#fff' }
});