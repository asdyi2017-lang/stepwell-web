...
import { Image, Platform, Pressable, ScrollView, StyleSheet, Text, View, Alert, Modal, TextInput, Share } from 'react-native';
...
Generated by StepWell System
  ...
  // НОВОЕ: Функция чтобы поделиться достижением
  ...
  // НОВОЕ: Обработчик клика по иконке достижения
      ...
      <View style={styles.root}>
        ...
        <View style={styles.main}>
          <View style={styles.header}>
            <View style={{ flex: 1 }}>
              <Text style={styles.hTitle}>
                {page === 'doctor' && currentRole === 'doctor' ? 'Patients Directory' : 
                 currentRole === 'doctor' && selectedPatient ? `Patient: ${selectedPatient.first_name} ${selectedPatient.last_name}` :
              ...
              <View style={{flexDirection: 'row', alignItems: 'center', gap: 8}}>
                ...
                {currentRole === 'doctor' && selectedPatientId && (
                  <Pressable onPress={downloadReport} style={styles.downloadIconBtn}>
                    <Download size={18} color={colors.teal} />
                    <Text style={{fontSize: 12, fontWeight: '700', color: colors.teal}}>Export Report</Text>
                  </Pressable>
                )}
                {currentRole !== 'doctor' && snapshot && (
                  <Pressable onPress={downloadReport} style={styles.downloadIconBtn}>
                    <Download size={18} color={colors.teal} />
                    <Text style={{fontSize: 12, fontWeight: '700', color: colors.teal}}>Export My Data</Text>
                  </Pressable>
                )}
              </View>
            </View>
            ...
            >
              <LogOut size={26} color={colors.teal} />
            </Pressable>
          </View>
          ...
          {page === 'assistant' ? (
            ...
            <ScrollView contentContainerStyle={{ paddingBottom: 24 }}>
              ...
              {page === 'dashboard' && snapshot && (
                <View style={[styles.grid, !isWeb && styles.gridMobile]}>
                  <BiomarkerCard
